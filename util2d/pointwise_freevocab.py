...
class OSM():
    ...
    def get_pred_class_and_prob(self, image, binary_mask, cache_flag=True):
        ...
        qformer_feat = self.class_generator.generate_1n_avg_qformer(
            img_idd = [0, 0],
...
class FreeVocab_PointWise:
    ...
    def get_final_instance(self, scene_id, cfg):
        ...
        llm_feature_path = os.path.join(cfg.exp.save_dir, cfg.exp.exp_name, cfg.exp.llm_feature, scene_id)
        os.makedirs(llm_feature_path, exist_ok=True)
        
        llm_feature_path = os.path.join(llm_feature_path, scene_id + '.pth')
        flag = False # Save indicator
        if os.path.isfile(llm_feature_path):
            save_feature = torch.load(llm_feature_path)['feat']
            flag = True

        # Point level -> spp level
        ...
        # Feature bank
        feature_bank = torch.zeros((self.points.shape[0], 32, 768), dtype = torch.float16, device = 'cpu')
        frequency_counter = torch.zeros((self.points.shape[0], 1, 1), dtype=torch.float16, device='cpu')

        counter = 0
        for (obj_id, inst) in enumerate(tqdm(self.instance3d)):
            target = self.storage2d[obj_id]
            msk_set = target[0]['mask2d']
            check_mask_empty = [(msk_set[tt].sum() != 0) for tt in range(len(msk_set))]
            ...
            for i in range(len(images)):
                ...
                if (msk_set[i].sum().item() == 0):
                    continue
                msk = np.array(msk_set[i].cpu())
                if flag:
                    qformer_feat = save_feature[counter]
                    counter += 1
                else:
                    qformer_feat = self.freevocab_model.get_pred_class_and_prob(images[i], msk)
                    save_feature.append(qformer_feat)

                if qformer_feat == None:
                    continue

                feature_bank[self.instance3d[obj_id].cpu()] += qformer_feat.cpu()
                frequency_counter[self.instance3d[obj_id].cpu()] += 1 
        if flag == False:
            feature_dict = {'feat': save_feature}
            torch.save(feature_dict, llm_feature_path)            

        feature_bank /= frequency_counter
        ...
        for i in trange(instance3d.shape[0]):
            if i in self.unvalid:
                categories.append("None")
                confidences.append(1)
                continue

            # is_zero_vector = torch.all(feature_bank[instance3d[i]] == 0, dim=(1, 2))
            # # Filter out the zero vectors
            # filtered_tensor = feature_bank[instance3d[i]][~is_zero_vector]
            # # Calculate the mean, excluding zero vectors
            # mean_filtered = filtered_tensor.mean(dim=0) if filtered_tensor.numel() > 0 else torch.zeros(32, 768)
            mean_filtered = feature_bank[instance3d[i].cpu()]
            generated_output = self.freevocab_model.class_generator.generate_1n_avg_language(
                qformer_output=mean_filtered.to(TEST_DTYPE).to(self.device),
            ...
            breakpoint()
        return instance3d, confidences, categories
