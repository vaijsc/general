...
class FreeVocab_SPPWise:
    ...
    def get_final_instance(self, scene_id, cfg):
        ...
        # Feature bank
        feature_bank = torch.zeros((self.n_spp, 32, 768), dtype = torch.float64, device = self.device)
        frequency_counter = torch.zeros((self.n_spp, 1, 1), dtype=torch.float64, device=self.device)
        ...
        for (obj_id, inst) in enumerate(tqdm(self.instance3d)):
            ...
            for i in range(len(images)):
                ...
                feature_bank[instance3d_spp[obj_id]] += qformer_feat
                frequency_counter[instance3d_spp[obj_id]] += 1 
        if flag == False:
        ...
        instance3d_spp = custom_scatter_mean(
            instance3d.to(self.device),
            self.spp[None, :].expand(len(instance3d), -1),
            dim=-1,
            pool=True,
            output_type=torch.float64,
            )
        ...
        for i in trange(instance3d.shape[0]):
            ...
            is_zero_vector = torch.all(feature_bank[instance3d_spp[i]] == 0, dim=(1, 2))
            # Filter out the zero vectors
            filtered_tensor = feature_bank[instance3d_spp[i]][~is_zero_vector]
            ...
            generated_output = self.freevocab_model.class_generator.decode_qformer(
                qformer_output=mean_filtered.unsqueeze(0).to(TEST_DTYPE),
